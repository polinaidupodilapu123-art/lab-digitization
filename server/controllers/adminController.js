...
exports.uploadMasterData = async (req, res) => {
  try {
    ...
    if (type === 'students') {
      ...
      colleges.forEach(c => { collegeMap[normalizeCode(c.collegeCode)] = c._id; });
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        if (type === 'students') {
          ...
          const collegeId = collegeMap[normalizeCode(collegeCode)] || null;
...
exports.assignSubjects = async (req, res) => {
  try {
    const { studentIds, subjectIds, pagesRequired, academicYear, deadline } = req.body;
